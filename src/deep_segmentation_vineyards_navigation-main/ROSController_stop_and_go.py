...
def image_callback(data):
	global buffer_counter, output_buffer,bridge,pub,depth_flag,depth_frame,t0,stop
	message = Twist()
	# print(distance(start_odometry_x,start_odometry_y,current_odometry_x,current_odometry_y))
	# print(stop)
	if (distance(start_odometry_x,start_odometry_y,current_odometry_x,current_odometry_y) < (stop + 0.05)) and (distance(start_odometry_x,start_odometry_y,current_odometry_x,current_odometry_y) > (stop - 0.05)):
		stop += 1
		t_end = time.time() + 5
		print("Stopping")
